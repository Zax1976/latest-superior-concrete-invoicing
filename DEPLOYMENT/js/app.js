...
(function() {
    ...
    window.addEventListener('error', function(event) {
        ...
        // Suppress SW 404 errors on GitHub Pages
        if (window.location.hostname.endsWith('github.io')) {
            const msg = String(event.message || '').toLowerCase();
            const filename = String(event.filename || '').toLowerCase();
            
            if (msg.includes('sw.js') || filename.includes('sw.js') ||
                msg.includes('service worker') || msg.includes('serviceworker') ||
                msg.includes('404') && (msg.includes('sw') || msg.includes('service'))) {
                console.log('[PAGES:SW_SUPPRESSED] 404 sw.js or service worker error');
                event.preventDefault();
                return;
            }
        }
    ...
    window.addEventListener('unhandledrejection', function(event) {
        // === SAFE-HOTFIX:PAGES-FIRST-PAINT (BEGIN)
        // Suppress transient promise rejections on GitHub Pages first paint
        if (window.location.hostname.endsWith('github.io') && 
            sessionStorage.getItem('PAGES_FIRST_PAINT_OK') !== '1') {
            const reason = String(event.reason || '').toLowerCase();
            
            if (reason.includes('sw.js') || reason.includes('service worker') ||
                reason.includes('message channel') || reason.includes('listener indicated') ||
                reason.includes('404')) {
                console.log('[PAGES:SW_SUPPRESSED] Promise rejection for SW/transient error');
                event.preventDefault();
                return;
            }
        }
    ...
    const App = {
        ...
        showInvoicePreview: function(invoice) {
            ...
            return this.showView('invoice-preview').then(() => {
                try {
                    ...
                    // Pre-generate PDF for email attachment - happens after preview, not on click
                    setTimeout(() => {
                        if (window.PDFGenerator && window.EmailAttachmentCache) {
                            try {
                                // === SAFE-HOTFIX: EMAIL_PRERENDER_CACHE (BEGIN)
                                console.log('[EMAIL:ATTACH] prerender start', { kind: 'invoice', id: invoice.id });
                                const result = window.PDFGenerator.renderToBlob('invoice', invoice.id);
                                // Check if result is valid (not an error object)
                                if (result && result.blob && !result.reason) {
                                    window.EmailAttachmentCache.set(
                                        'invoice', 
                                        invoice.id, 
                                        invoice.number,
                                        result.blob,
                                    ...
                                    console.log('[EMAIL:ATTACH] prerender cached', { 
                                        kind: 'invoice',
                                        id: invoice.id,
                                    ...
                                    console.log('[EMAIL:ATTACH] prerender skip', { reason: result.reason });
                                }
                                // === SAFE-HOTFIX: EMAIL_PRERENDER_CACHE (END)
                            } catch (e) {
                                console.log('[EMAIL:ATTACH] prerender error', e.message);
                            }
                        }
        ...
        showEstimateCreation: function(businessType = null) {
            // === SAFE-HOTFIX:PAGES-FIRST-PAINT (BEGIN)
            console.log('[PAGES:VIEW] estimate activation starting');
            // === SAFE-HOTFIX:PAGES-FIRST-PAINT (END)
            
            return this.showView('estimate-creation').then(() => {
                try {
                    // === SAFE-HOTFIX:PAGES-FIRST-PAINT (BEGIN)
                    // Race guard: wait for DOM elements to be ready
                    const waitForEstimateDOM = (retries = 10) => {
                        const domReady = {
                            view: !!document.querySelector('#estimate-creation.view.active'),
                            lengthInput: !!document.getElementById('estimate-length-ft'),
                            widthInput: !!document.getElementById('estimate-width-ft'),
                            depthInput: !!document.getElementById('estimate-depth-in'),
                            businessRadio: !!document.querySelector('input[name="estimateBusinessType"]'),
                            form: !!document.getElementById('estimate-form')
                        };
                        
                        console.log('[PAGES:DOM]', domReady);
                        
                        // If critical elements missing and retries left, wait and retry
                        if ((!domReady.view || !domReady.businessRadio || !domReady.form) && retries > 0) {
                            setTimeout(() => waitForEstimateDOM(retries - 1), 100);
                            return;
                        }
                        
                        // Mark first paint as OK once view is ready
                        if (domReady.view && window.location.hostname.endsWith('github.io')) {
                            sessionStorage.setItem('PAGES_FIRST_PAINT_OK', '1');
                            console.log('[PAGES:READY] handlers can bind safely');
                        }
                ...
                try {
                    ...
                    setTimeout(() => {
                        if (window.PDFGenerator && window.EmailAttachmentCache) {
                            try {
                                // === SAFE-HOTFIX: EMAIL_PRERENDER_CACHE (BEGIN)
                                console.log('[EMAIL:ATTACH] prerender start', { kind: 'estimate', id: estimate.id });
                                const result = window.PDFGenerator.renderToBlob('estimate', estimate.id);
                                // Check if result is valid (not an error object)
                                if (result && result.blob && !result.reason) {
                                    window.EmailAttachmentCache.set(
                                        'estimate', 
        ...
        setupAutoSave: function() {
            try {
                ...
                // === SAFE-HOTFIX:NATIVE_EMAIL_UNLOAD_BYPASS (BEGIN)
                // Global beforeunload bypass for email compose
                window.__UNLOAD_BYPASS__ = {
                    enabled: false,
                    until: 0
                };
                
                window.enableUnloadBypass = function(ttlMs = 3000) {
                    window.__UNLOAD_BYPASS__.enabled = true;
                    window.__UNLOAD_BYPASS__.until = Date.now() + ttlMs;
                    console.log('[UNLOAD:BYPASS] enabled', { ttl: ttlMs });
                    
                    // Auto-expire on visibility change or timeout
                    setTimeout(() => {
                        if (window.__UNLOAD_BYPASS__.enabled) {
                            window.__UNLOAD_BYPASS__.enabled = false;
                            console.log('[UNLOAD:BYPASS] expired');
                        }
                    }, ttlMs);
                };
                
                window.disableUnloadBypass = function() {
                    window.__UNLOAD_BYPASS__.enabled = false;
                    window.__UNLOAD_BYPASS__.until = 0;
                };
                
                window.isUnloadBypassActive = function() {
                    return window.__UNLOAD_BYPASS__.enabled && 
                           Date.now() < window.__UNLOAD_BYPASS__.until;
                };
                
                // Listen for visibility change to consume bypass
                document.addEventListener('visibilitychange', () => {
                    if (document.hidden && window.__UNLOAD_BYPASS__.enabled) {
                        window.__UNLOAD_BYPASS__.enabled = false;
                        console.log('[UNLOAD:BYPASS] consumed (visibilitychange hidden)');
                    }
                });
                // === SAFE-HOTFIX:NATIVE_EMAIL_UNLOAD_BYPASS (END)
                
                // Warn before leaving page with unsaved changes
                window.addEventListener('beforeunload', (e) => {
                    // === SAFE-HOTFIX:NATIVE_EMAIL_UNLOAD_BYPASS (BEGIN)
                    // Skip if bypass is active
                    if (window.isUnloadBypassActive && window.isUnloadBypassActive()) {
                        return; // Don't show unsaved changes prompt
                    }
                    // === SAFE-HOTFIX:NATIVE_EMAIL_UNLOAD_BYPASS (END)
    ...
    */
    ...
    window.emailInvoice = function() {
        // === SAFE-HOTFIX:EMAIL-STATE-SINGLE (BEGIN)
        // State management is handled by sendInvoiceEmail, not here
        // This prevents double state activation
        // === SAFE-HOTFIX:EMAIL-STATE-SINGLE (END)
    ...
    window.emailEstimate = function() {
        // === SAFE-HOTFIX:EMAIL-STATE-SINGLE (BEGIN)
        // State management is handled by sendEstimateEmail, not here
        // This prevents double state activation
        // === SAFE-HOTFIX:EMAIL-STATE-SINGLE (END)
        
        if (window.EmailService && window.EstimateManager) {
            const estimate = window.EstimateManager.previewedEstimate || window.EstimateManager.currentEstimate;
