    ...
    const App = {
        ...
        loadData: function() {
            try {
                ...
                // Ensure all invoices have numbers
                this.ensureInvoiceNumbers();
        ...
        ensureInvoiceNumbers: function() {
            try {
                let needsSave = false;
                let currentNumber = 1;
                
                // Sort invoices by date to assign numbers chronologically
                AppState.invoices.sort((a, b) => new Date(a.date) - new Date(b.date));
                
                AppState.invoices.forEach(invoice => {
                    if (!invoice.number) {
                        invoice.number = currentNumber++;
                        needsSave = true;
                    } else {
                        currentNumber = Math.max(currentNumber, invoice.number + 1);
                    }
                });
                
                // Update next invoice number if needed
                if (currentNumber > AppState.nextInvoiceNumber) {
                    AppState.nextInvoiceNumber = currentNumber;
                    needsSave = true;
                }
                ...
                if (needsSave) {
                    this.saveData();
                    console.log('Updated invoice numbers for existing invoices');
                }
                
            } catch (error) {
                ErrorHandler.log(error, 'Ensure Invoice Numbers');
            }
        },
                ...
                const invoicesHtml = recentInvoices.map(invoice => `
                    <div class="invoice-item">
                        <div class="invoice-info">
                            <h4>Invoice #${invoice.number || 'DRAFT'}</h4>
