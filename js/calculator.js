...
(function() {
    ...
    const ConcreteCalculator = {
        ...
        // === VERSION 9 FIX: Better estimate context handling
        getCalcInputs: function(rootElement) {
            // Check if we're in estimate context
            const isEstimate = document.querySelector('input[name="estimateBusinessType"][value="concrete"]')?.checked || 
                              document.querySelector('#estimate-creation.view.active');
            
            if (isEstimate) {
                console.log('[V9-FIX] Detected estimate context in getCalcInputs');
            }
            ...
            // Scoped helper functions with better error handling
            const numFromScoped = (id) => {
                // Try scoped search first
                let el = calcRoot.querySelector(`#${id}`);
                
                // If not found in scope and we're in estimate, try global search
                if (!el && isEstimate) {
                    el = document.getElementById(id);
                    if (el) {
                        console.log(`[V9-FIX] Found ${id} globally for estimate`);
                    }
                }
                
                const value = el ? parseFloat(el.value) || 0 : 0;
                console.log(`[INPUT-SCOPE:READ] #${id} = ${value} (found: ${!!el})`);
                return value;
            };
            ...
            // === VERSION 9 FIX: Log warning if inputs are missing in estimate mode
            if (isEstimate && (inputs.length === 0 || inputs.width === 0)) {
                console.warn('[V9-FIX] Calculator inputs not found in estimate mode. Calculator may not be loaded.');
                // Check if calculator elements exist at all
                const calcLengthExists = !!document.getElementById('calc-length');
                const calcWidthExists = !!document.getElementById('calc-width');
                console.log('[V9-FIX] Calculator elements exist:', { 
                    'calc-length': calcLengthExists, 
                    'calc-width': calcWidthExists 
                });
            }
            ...
            try {
                ...
                if (calculateBtn && !calculateBtn.hasAttribute('data-calc-bound')) {
                    ...
                    calculateBtn.addEventListener('click', (event) => {
                        ...
                        // === VERSION 9 FIX: Better error handling for estimate context
                        // Check if we're in estimate context
                        const isEstimate = document.querySelector('input[name="estimateBusinessType"][value="concrete"]')?.checked || 
                                          document.querySelector('#estimate-creation.view.active');
                        
                        // Validate inputs with context-aware messaging
                        if (i.length <= 0 || i.width <= 0) {
                            if (isEstimate) {
                                // Check if calculator inputs exist at all
                                const calcLengthExists = !!document.getElementById('calc-length');
                                const calcWidthExists = !!document.getElementById('calc-width');
                                
                                if (!calcLengthExists || !calcWidthExists) {
                                    console.error('[V9-FIX] Calculator not available in estimate mode');
                                    alert('The concrete calculator is not available for estimates at this time. Please manually enter the service details and price.');
                                } else {
                                    alert('Please enter valid dimensions (length and width must be greater than 0)');
                                }
                            } else {
                                // Invoice mode - standard message
                                alert('Please enter valid dimensions (length and width must be greater than 0)');
                            }
                            return;
                        }
                        ...
                        if (window.ERPolyEstimator && window.ERPolyEstimator.estimate) {
                            ...
                            // === VERSION 9.16 FIX: Store length and width in currentCalculation
                            // Store result with dimensions and update display
                            this.currentCalculation = {
                                ...result,
                                length: i.length,
                                width: i.width
                            ...
                            // Ensure global storage for estimate mode with dimensions
                            if (window.ConcreteCalculator) {
                                window.ConcreteCalculator.currentCalculation = this.currentCalculation;
                                console.log('[V9.16] Stored calculation with dimensions globally');
                            }
                            ...
                            if (resultsDiv) {
                                ...
                                // === VERSION 9.3 FIX: Write dataset for BOTH contexts (not just estimate)
                                if (resultsDiv) {
                                    resultsDiv.dataset.priceLow = priceLow;
                                    resultsDiv.dataset.priceMid = priceMid;
                                    resultsDiv.dataset.priceHigh = priceHigh;
                                    console.log('[V9.3] Set dataset on calc-results for context:', context, resultsDiv.dataset);
                                }
                            ...
                            // === VERSION 9.16 FIX: Store result with dimensions
                            this.currentCalculation = {
                                ...result,
                                length: i.length,
                                width: i.width
                            ...
                            // Ensure global storage for estimate fallback path with dimensions
                            if (window.ConcreteCalculator) {
                                window.ConcreteCalculator.currentCalculation = this.currentCalculation;
                                console.log('[V9.16] Stored calculation with dimensions globally (fallback path)');
                            }
                            ...
                            if (resultsDiv && result) {
                                ...
                                // === VERSION 9.3 FIX: Write dataset for BOTH contexts (fallback path)
                                if (resultsDiv) {
                                    resultsDiv.dataset.priceLow = priceLowVal;
                                    resultsDiv.dataset.priceMid = priceMidVal;
                                    resultsDiv.dataset.priceHigh = priceHighVal;
                                    console.log('[V9.3-FALLBACK] Set dataset on calc-results for context:', context, resultsDiv.dataset);
                                }
        ...
        addConcreteService: function() {
            try {
                ...
                if (window.showPriceReview) {
                    ...
                    // Check if Use Selected Price button exists - if so, let user handle it
                    const useButton = document.getElementById('use-selected-price');
                    if (useButton) {
                        console.log('[CALCULATOR] Use Selected Price button found - user will handle pricing');
                        // Don't add service automatically - wait for user to click button
                        // Store calculation for later use by button handler
                        window.ConcreteCalculator.currentCalculation = calc;
                        return;
                    }
                    
                    // Only use fallback if no button exists (shouldn't happen in normal flow)
                    console.log('[CALCULATOR] No price review and no button - using fallback');
                    this.addConcreteServiceWithCustomPrice({
                        ...calc,
                        customPrice: calc.recommendedPrice,
                        finalPrice: calc.recommendedPrice,
                        priceOverride: false
                    });
                }
        ...
        addConcreteServiceWithCustomPrice: function(serviceData) {
            try {
                ...
                // === VERSION 9.14 FIX: Add dimensions property for display
                // Format dimensions like "10 x 20 x 1" 
                ...
                const service = {
                    ...
                    dimensions: dimensionsText, // V9.14: Store dimensions separately for display
        ...
        getServiceDescription: function(calc) {
            try {
                ...
                // === VERSION 9.16 FIX: Removed squareFootage display since dimensions are shown separately
