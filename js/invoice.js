...
window.InvoiceManager = window.InvoiceManager || {
    ...
    // === SAFE-HOTFIX: INV_MANAGER_API (BEGIN) - Add API methods early
    getInvoices: function() {
        // Early fallback implementation
        if (window.StorageManager && typeof window.StorageManager.getInvoices === 'function') {
            return window.StorageManager.getInvoices() || [];
        } else if (window.App && window.App.AppState && window.App.AppState.invoices) {
            return window.App.AppState.invoices || [];
        } else {
            try {
    ...
    previewedInvoice: null,
    // === SAFE-HOTFIX: INV_MANAGER_API (END)
    ...
    const InvoiceManager = {
        ...
        addServiceToList: function(service) {
            try {
                ...
                if (service.details && service.details.jobPricing) {
                    // Masonry services - show only Quantity and Amount
                    ...
                    serviceItem.innerHTML = `
                        <div class="service-row-masonry" style="display: flex; justify-content: space-between; align-items: center; padding: 10px 0; border-bottom: 1px solid #ddd;">
                            <div class="service-quantity" style="flex: 1;">
                                <strong>Quantity:</strong> ${service.quantity || 1}
                            </div>
                            <div class="service-amount-container" style="flex: 1; text-align: right;">
                    ...
                    let serviceDetailsHtml = `<p>${service.quantity} ${service.unit} × ${this.formatCurrency(service.rate)}</p>`;
        ...
        generateInvoicePreview: function(invoice) {
            try {
                ...
                console.log('[INVOICE:PREVIEW_SET]', { id: invoice.id, number: invoice.number });
    ...
    const EstimateManager = {
        ...
        getConcreteServiceDescription: function(calc) {
            try {
                ...
                let description = projectTypes[calc.projectType] || 'Concrete Leveling';
        ...
        addServiceToList: function(service) {
            try {
                ...
                if (isMasonryService) {
                    // Masonry services - show only Quantity and Amount columns
                    ...
                    if (service.details && service.details.jobPricing) {
                        ...
                        // Quantity-based pricing (concrete)
                        serviceDetailsHtml = `<p>${service.quantity} ${service.unit} × ${this.formatCurrency(service.rate)}</p>`;
                    }
        ...
        handleEstimateSubmission: function() {
            try {
                ...
                if (window.App) {
                    window.App.showError('Failed to create estimate. Please try again.');
                }
        ...
        collectFormData: function() {
            try {
                ...
                this.currentEstimate.notes = document.getElementById('estimate-notes').value.trim();
        ...
        populateFormWithEstimate: function(estimate) {
            try {
                // Basic fields
                this.setFieldValue('estimate-customer-name', estimate.customerName);
                this.setFieldValue('estimate-customer-email', estimate.customerEmail);
                this.setFieldValue('estimate-customer-phone', estimate.customerPhone);
...
if (window.JStarkInvoicing && window.JStarkInvoicing.InvoiceManager) {
    // === SAFE-HOTFIX: INV_MANAGER_API (BEGIN)
    // Store early API methods before overwriting
    const earlyMethods = {};
    if (window.InvoiceManager) {
        if (window.InvoiceManager.getInvoices) earlyMethods.getInvoices = window.InvoiceManager.getInvoices;
        if (window.InvoiceManager.getInvoiceById) earlyMethods.getInvoiceById = window.InvoiceManager.getInvoiceById;
        if (window.InvoiceManager.hasOwnProperty('previewedInvoice')) earlyMethods.previewedInvoice = window.InvoiceManager.previewedInvoice;
    }
    // === SAFE-HOTFIX: INV_MANAGER_API (END)
    ...
    // === SAFE-HOTFIX: INV_MANAGER_API (BEGIN)
    // Restore/ensure API methods exist
    if (!window.InvoiceManager.getInvoices) {
        window.InvoiceManager.getInvoices = earlyMethods.getInvoices || function() {
            if (window.StorageManager && typeof window.StorageManager.getInvoices === 'function') {
                return window.StorageManager.getInvoices() || [];
            } else if (window.App && window.App.AppState && window.App.AppState.invoices) {
                return window.App.AppState.invoices || [];
    ...
    if (!window.InvoiceManager.getInvoiceById) {
        window.InvoiceManager.getInvoiceById = earlyMethods.getInvoiceById || function(id) {
            const invoices = this.getInvoices();
            return invoices.find(inv => inv.id === id) || null;
        };
    }
    
    if (!window.InvoiceManager.hasOwnProperty('previewedInvoice')) {
        window.InvoiceManager.previewedInvoice = earlyMethods.previewedInvoice || null;
    }
    
    // Log API readiness
    console.log('[INV-API] ready', {
        hasGetInvoices: typeof window.InvoiceManager.getInvoices === 'function',
        hasGetInvoiceById: typeof window.InvoiceManager.getInvoiceById === 'function',
        hasPreviewedInvoice: window.InvoiceManager.hasOwnProperty('previewedInvoice')
    });
    // === SAFE-HOTFIX: INV_MANAGER_API (END)
}
...
if (window.JStarkInvoicing && window.JStarkInvoicing.EstimateManager) {
    ...
    // === SAFE-HOTFIX: ESTIMATE_MANAGER_API (BEGIN)
    // Add missing API methods that pdf.js and other components depend on
    if (!window.EstimateManager.getEstimates) {
        window.EstimateManager.getEstimates = function() {
            // Try multiple sources for estimates
            if (window.StorageManager && typeof window.StorageManager.getEstimates === 'function') {
                return window.StorageManager.getEstimates() || [];
            } else if (window.App && window.App.AppState && window.App.AppState.estimates) {
                return window.App.AppState.estimates || [];
            } else {
                ...
                try {
                    ...
                    console.error('[EST-API] Error loading estimates:', e);
                    return [];
                }
            }
        };
    ...
    if (!window.EstimateManager.getEstimateById) {
        window.EstimateManager.getEstimateById = function(id) {
            const estimates = this.getEstimates();
            return estimates.find(est => est.id === id) || null;
        };
    }
    
    // Ensure previewedEstimate property exists
    if (!window.EstimateManager.hasOwnProperty('previewedEstimate')) {
        window.EstimateManager.previewedEstimate = null;
    }
    
    // Log API readiness
    console.log('[EST-API] ready', {
        hasGetEstimates: typeof window.EstimateManager.getEstimates === 'function',
        hasGetEstimateById: typeof window.EstimateManager.getEstimateById === 'function',
        hasPreviewedEstimate: window.EstimateManager.hasOwnProperty('previewedEstimate')
    });
    // === SAFE-HOTFIX: ESTIMATE_MANAGER_API (END)
    ...
    window.EstimateManager.addMasonryEstimateService = function() {
        ...
        const serviceType = document.getElementById('estimate-masonry-service')?.value;
        const description = document.getElementById('estimate-masonry-description')?.value?.trim();
        const rawPrice = document.getElementById('estimate-masonry-price')?.value?.trim();
        ...
        document.getElementById('estimate-masonry-description').value = '';
        document.getElementById('estimate-masonry-price').value = '';
