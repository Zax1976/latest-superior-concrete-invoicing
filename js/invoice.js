...
(function() {
    ...
    const InvoiceManager = {
        ...
        validateInvoiceForm: function() {
            try {
                // Use the enhanced FormValidator if available
                if (window.FormValidator && window.FormValidator.validateInvoiceForm) {
                    return window.FormValidator.validateInvoiceForm();
                }
                
                // Fallback to basic validation
                return this.basicValidateInvoiceForm();
                
            } catch (error) {
                console.error('Form validation error:', error);
                if (window.ErrorHandler) {
                    window.ErrorHandler.log(error, 'Form Validation', 'error');
                }
                return false;
            }
        },
        
        basicValidateInvoiceForm: function() {
            try {
                let isValid = true;
                const errors = [];
                ...
                if (emailField && emailField.value.trim()) {
                    ...
                    if (window.SecurityUtils && !window.SecurityUtils.validateEmail(email)) {
                ...
                if (phoneField && phoneField.value.trim()) {
                    ...
                    if (window.SecurityUtils && !window.SecurityUtils.validatePhone(phone)) {
                ...
                this.currentInvoice.services.forEach((service, index) => {
                    ...
                    if (!service.rate || service.rate < 0) {
                        errors.push(`Service ${index + 1}: Rate must be 0 or greater`);
                        isValid = false;
                    }
                });
                ...
                if (!isValid) {
                    if (window.ErrorHandler && window.ErrorHandler.showUserError) {
                        ...
                        window.ErrorHandler.showUserError(errorMessage);
                    } else {
                        const errorMessage = 'Please fix the following errors:\\n\\n• ' + errors.join('\\n• ');
                        alert(errorMessage);
                    }
                ...
                console.error('Basic form validation error:', error);
                return false;
            }
        },
        ...
        collectFormData: function() {
            try {
                // Get form values with security sanitization
                const businessTypeRadio = document.querySelector('input[name="businessType"]:checked');
                this.currentInvoice.businessType = businessTypeRadio ? businessTypeRadio.value : '';
                
                // Use SecurityUtils to sanitize input data
                const rawData = {
                ...
                if (window.SecurityUtils) {
                    this.currentInvoice.customerName = window.SecurityUtils.sanitizeInput(rawData.customerName);
                    
                    // Validate email if provided
                    if (rawData.customerEmail && !window.SecurityUtils.validateEmail(rawData.customerEmail)) {
                        throw new Error('Invalid email address format');
                    }
                    this.currentInvoice.customerEmail = window.SecurityUtils.sanitizeInput(rawData.customerEmail);
                    
                    // Validate phone if provided
                    if (rawData.customerPhone && !window.SecurityUtils.validatePhone(rawData.customerPhone)) {
                        throw new Error('Invalid phone number format');
                    }
                    this.currentInvoice.customerPhone = window.SecurityUtils.sanitizeInput(rawData.customerPhone);
                    
                    this.currentInvoice.customerAddress = window.SecurityUtils.sanitizeInput(rawData.customerAddress);
                } else {
                    // Fallback if SecurityUtils not available
                    this.currentInvoice.customerName = rawData.customerName;
                    this.currentInvoice.customerEmail = rawData.customerEmail;
                    this.currentInvoice.customerPhone = rawData.customerPhone;
                    this.currentInvoice.customerAddress = rawData.customerAddress;
                }
                
                // Date validation
                const dateInput = document.getElementById('invoice-date').value;
                this.currentInvoice.date = dateInput || new Date().toISOString().split('T')[0];
                
                // Sanitize notes and status
                const rawNotes = document.getElementById('invoice-notes').value.trim();
                const rawStatus = document.getElementById('invoice-status').value;
                
                if (window.SecurityUtils) {
                    this.currentInvoice.notes = window.SecurityUtils.sanitizeInput(rawNotes);
                    this.currentInvoice.status = window.SecurityUtils.sanitizeInput(rawStatus);
                } else {
                    this.currentInvoice.notes = rawNotes;
                    this.currentInvoice.status = rawStatus;
                }
        ...
        saveInvoice: function() {
            try {
                // Use StorageManager if available to ensure customer data is updated
                if (window.StorageManager && window.StorageManager.saveInvoice) {
                    const success = window.StorageManager.saveInvoice(this.currentInvoice);
                    if (success) {
                        // Update AppState to reflect the changes
                        if (window.App && window.App.AppState) {
                            window.App.AppState.invoices = window.StorageManager.loadInvoices();
                            window.App.AppState.customers = window.StorageManager.loadCustomers();
                        }
                    }
                    return success;
