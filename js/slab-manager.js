...
window.SlabManager = (function() {
    ...
    function onUseSelectedPriceClick(evt) {
        ...
        // Get tier from scoped root (check for priceSelection first, then price-tier)
        let tier = root.querySelector('input[name="priceSelection"]:checked')?.value || 
        ...
        if (list) {
            ...
            if (context === 'invoice') {
                ...
                serviceRow.innerHTML = `
                    <td>${line.type === 'concrete' ? 'Concrete Leveling' : 'Service'}</td>
                    <td>${description}</td>
                    <td>1</td>
