...
(function() {
    ...
    const StorageManager = {
        ...
        // === VERSION 9.23: Add missing addInvoice method that invoice.js expects
        addInvoice: function(invoice) {
            try {
                ...
                const currentNumber = parseInt(localStorage.getItem(this.keys.nextInvoiceNumber) || '1');
                const nextNumber = currentNumber + 1;
                localStorage.setItem(this.keys.nextInvoiceNumber, String(nextNumber));
                
                // Also update App.AppState if available
                if (window.App && window.App.AppState) {
                    window.App.AppState.nextInvoiceNumber = nextNumber;
                }
                
                console.log(`✅ Invoice saved with number ${invoice.number}, next will be ${nextNumber}`);
                return true;
            } catch (error) {
        ...
        // === SAFE-HOTFIX: ESTIMATE SAVE→LIST→PREVIEW→PDF (BEGIN)
        // Estimate management
        saveEstimates: function(estimates) {
            try {
        ...
        getEstimates: function() {
            try {
                const data = localStorage.getItem(this.keys.estimates);
                return data ? JSON.parse(data) : [];
            } catch (error) {
        ...
        saveNextEstimateNumber: function(number) {
            try {
                localStorage.setItem(this.keys.nextEstimateNumber, String(number));
                return true;
            } catch (error) {
        ...
        getNextEstimateNumber: function() {
            try {
                const num = localStorage.getItem(this.keys.nextEstimateNumber);
        ...
        // === VERSION 9.23: Add missing addEstimate method for consistency
        addEstimate: function(estimate) {
            try {
                // Save the estimate to the array
                const estimates = this.getEstimates();
                ...
                const saved = this.saveEstimates(estimates);
                if (!saved) {
                    return false;
                }
                
                // Increment the next estimate number if it's a new estimate
                if (existingIndex < 0) {
                    const currentNumber = this.getNextEstimateNumber();
                    const nextNumber = currentNumber + 1;
                    this.saveNextEstimateNumber(nextNumber);
                    
                    // Also update App.AppState if available
                    if (window.App && window.App.AppState) {
                        window.App.AppState.nextEstimateNumber = nextNumber;
                    }
                    
                    console.log(`✅ Estimate saved with number ${estimate.number}, next will be EST-${String(nextNumber).padStart(4, '0')}`);
                }
